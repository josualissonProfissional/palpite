-- Versao simplificada: em caso de empate total (todos os candidatos
-- da posicao tem o mesmo numero de votos), todos contam como acerto.
...
begin
  ...
  -- PONTUACAO COM EMPATE: se todos os jogadores de uma posicao tem
  -- o mesmo numero de votos (ex: 2 pessoas, 1 voto cada), nao ha
  -- "maioria" real. Nesse caso, todos contam como acerto.
  insert into palpite.best_player_scores (window_id, group_id, user_id, hits, points)
  with position_max as (
    select selected_role, max(daily_votes) as max_votes
    from pg_temp.best_player_daily_candidates
    group by selected_role
  ),
  position_min as (
    select selected_role, min(daily_votes) as min_votes
    from pg_temp.best_player_daily_candidates
    group by selected_role
  ),
  is_tied_position as (
    select pm.selected_role
    from position_max pm
    join position_min pn on pn.selected_role = pm.selected_role
    where pm.max_votes = pn.min_votes
  ),
  ballot_players as (
  ...
  scored as (
    select
      ...
      bp.selected_role,
      exists (
        select 1 from palpite.best_player_results r
        where r.window_id = p_window_id
          and r.player_id = bp.player_id
          and r.selected_role = bp.selected_role
      )
      or exists (
        select 1 from is_tied_position tp
        where tp.selected_role = bp.selected_role
      ) as is_hit
